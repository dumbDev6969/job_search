...
def check_value_exists(table_name: str, column: str, value: str) -> bool:
    """
    Check if a value exists in a specified table and column.
    ...
    try:
        db = DatabaseManager('mysql', 'mysql+pymysql://root@localhost/job_portal_db')
        query = text(f"SELECT COUNT(*) as count FROM {table_name} WHERE {column} = :value_compare OR {column} = :value_identify")

        # Execute the query with parameters dictionary
        result = db.execute_query(query, {'value_compare': '2', 'value_identify': '1'})
...
check_column_exists('employer_verification', 'status', '1')